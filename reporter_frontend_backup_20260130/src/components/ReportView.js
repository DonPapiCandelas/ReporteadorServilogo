...
// --- Sub-Componente: Tabla de Entradas (Paginada) ---
function EntriesTable({ entries }) {
  ...
  return (
    <div className="space-y-2">
        ...
        <table className="w-full mini-table">
          ...
          <tbody className="divide-y divide-border">
            ...
            {currentEntries.length === 0 && (
              <tr>
                <td colSpan="7" className="text-center py-4 text-text-sub">No entries found.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="flex justify-between items-center text-xs text-text-sub px-1">
          <span>Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, entries.length)} of {entries.length}</span>
          <div className="flex gap-2">
            <button
              onClick={() => handlePageChange(currentPage - 1)}
              disabled={currentPage === 1}
              className="px-2 py-1 bg-surface border border-border rounded hover:bg-surface-lighter disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Previous
            </button>
            <span className="flex items-center font-mono">Page {currentPage} of {totalPages}</span>
            <button
              onClick={() => handlePageChange(currentPage + 1)}
